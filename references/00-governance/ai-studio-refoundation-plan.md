...
- **Status Baseline**: `NOT_STARTED` (Sub-gate progression: AS5-A0 dan AS5-G1 berstatus `ACCEPTED`; AS5-G2 menjadi gerbang tata kelola berikutnya (*NEXT governance gate*), berstatus `NOT_STARTED`, dan implementasinya belum dimulai serta memerlukan otorisasi eksplisit terpisah dari Project Owner; AS5-G3 s.d. AS5-G5 tetap `NOT_STARTED`.)
...
- **Dekomposisi Gerbang Terbuka & Batas Otorisasi (*Approved Gated Decomposition*)**:
  ...
  - `AS5-G2 — Generic Project / ProductionDomain / Artifact Contract Foundation`: `NOT_STARTED; NEXT governance gate` (Menambahkan kontrak, skema, dan validator generic `Project`, `ProductionDomain`, dan `Artifact` pada Studio OS `adityaairlangga12/ai-game-dev-studio-os`; implementasi belum dimulai dan TIDAK diotorisasi oleh tata kelola ini; memerlukan otorisasi eksplisit terpisah dari Project Owner dan Work Order implementasi berbatas).
  - `AS5-G3 — Lentera Project Integration & Production Lifecycle Foundation`: `NOT_STARTED` (Mengimplementasikan deskriptor/konfigurasi proyek, perutean domain, dan fondasi rekam jejak siklus hidup berbasis berkas lokal proyek di `adityaairlangga12/lentera-pudar`. Tanpa eksekusi control-plane eksternal; tanpa dispatch Blender/Unreal).
...
- **Aturan Transisi & Pembatasan Otorisasi**:
  ...
  - AS5-A0 dan AS5-G1 telah selesai dan berstatus `ACCEPTED`; AS5-G2 menjadi gerbang tata kelola berikutnya (*NEXT governance gate*), berstatus `NOT_STARTED`, dan `TIDAK DIOTORISASI` untuk implementasi oleh penerimaan G1; gerbang AS5-G3 s.d. AS5-G5 tetap `NOT_STARTED` dan belum diotorisasi.
  ...
  - Pasca-penerimaan AS5-G1, implementasi AS5-G2 tetap `NOT_STARTED`; AS6–AS8 tetap `NOT_STARTED`; Phase H1 tetap `NOT_STARTED` dan terblokir (*BLOCKED*) hingga AS8 diterima secara eksternal dan project status membukanya secara terpisah.
